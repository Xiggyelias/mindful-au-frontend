...
export function confirmProfileAnonymousModeTransition(currentValue: unknown, nextChecked: boolean): boolean {
  const current = isProfileAnonymousMode(currentValue);
  if (current && !nextChecked) {
    return window.confirm(PROFILE_ANON_MODE_OFF_CONFIRMATION);
  }
  return true;
}
