...
const AdminLogin = () => {
  ...
  const { user, role, signInWithEmail, signOut, isLoading: authLoading } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error, user: loggedInUser } = await signInWithEmail(
      formData.email.trim(),
      formData.password
    );
    ...
    const hasAdminRole = loggedInUser?.roles?.some((r) => r.role === "admin");
    if (!hasAdminRole) {
    ...
    toast.success("Welcome back, Administrator! Redirecting to dashboard...");
    navigate("/admin/dashboard", { replace: true });
    setIsLoading(false);
  };
        ...
        <main className="flex-1 flex items-center justify-center px-4 py-8">
          <Card variant="glass" className="w-full max-w-md opacity-0 animate-slide-up">
            <CardHeader className="text-center">
              ...
              <CardDescription>
                Access system administration and oversight tools
              </CardDescription>
            </CardHeader>
            <CardContent>
